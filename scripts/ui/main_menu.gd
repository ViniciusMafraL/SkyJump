...
## Mostra as estrelas e a sequência do Desafio Diário e abre o calendário do modo.
...
@export var login_reward_caption: String = "RECOMPENSA DIÁRIA"
...
func _show_pending_login_reward() -> void:
	...
	if amount > 0:
		_login_feedback.show_reward(amount, login_reward_caption)


func _on_stars_changed(_total: int, _delta: int, reason: StringName) -> void:
	if reason == StarWallet.REASON_LOGIN:
		_show_pending_login_reward()
